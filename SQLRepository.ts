...
export async function getGomiWorkers(): Promise<Member[]> {
  ...
  const [rows, ]: any[] = await db.query("SELECT `members`.* FROM `members` JOIN `trash` ON `members`.`slackID` = `trash`.`slackID` WHERE `on_duty` = true");
...
export async function updateGomiWorkers(): Promise<Member[] | null> {
  ...
  const [rows,]: any[] = await db.query("SELECT `members`.* FROM `members` JOIN `trash` ON `members`.`slackID` = `trash`.`slackID` WHERE `trash`.`done_in_loop` = FALSE");
